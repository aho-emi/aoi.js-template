...
class Handler {
  ...
  loadCommands(path) {
    ...
    const cmds = [];
    readdirSync(path).map(x => x.endsWith('.js') ? cmds.push(path + x) : readdirSync(path + x).forEach(y => y.endsWith('.js') ? cmds.push(path + x + '/' + y) : readdirSync(path + x + '/' + y).forEach(z => cmds.push(path + x + '/' + y + '/' + z))));
